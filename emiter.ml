...
| TELam (p, b) -> 
  ...
  let nenv = {
    ...
    oldlocals = SMap.union (fun _ n _ -> Some n) env.locals 
      (SMap.map (fun v -> v - 8 * (SMap.cardinal a + 2 + SMap.cardinal env.locals)) 
        env.oldlocals);
    captured = Hashtbl.create 17} in
  let b, fp = alloc_block nenv fpcur b in
  Hashtbl.iter (fun k v -> if not (SMap.mem k env.locals) then 
    Hashtbl.add env.captured k v) nenv.captured;
  let clos = { fp = fp; captured = nenv.captured } in
  AELam (clos, b), fpcur
| TECall (TCVar v, l) -> 
...
let rec compile_expr = function
  ...
  | AEOp (o, l)->
    ...
    | BMul ->
      getb e ++
      imulq (ind ~ofs:1 rbx) (ind ~ofs:1 rax) ++
      movq !%rax !%r14 ++
      mkint !%r14 
    | BDiv ->
      getb e ++
